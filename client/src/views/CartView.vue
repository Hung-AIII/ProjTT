    ...
    <div v-if="cartStore.items.length > 0">
      ...
      <div class="bg-dark3/80 backdrop-blur-lg border border-white/5 rounded-lg p-6 mt-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex gap-3">
          <button @click="clearCart" class="border border-gold text-light px-6 py-2 rounded hover:bg-gold hover:text-dark transition-all text-sm">Xóa hết</button>
          <router-link to="/checkout" class="bg-gold text-dark px-6 py-2 rounded hover:bg-goldHover transition-all text-sm font-semibold">Thanh toán</router-link>
        </div>
      </div>
    </div>
...
const updateQuantity = (id, qty) => cartStore.updateQuantity(id, qty)
const removeItem = (id) => { if (confirm('Xóa món này khỏi giỏ?')) cartStore.removeFromCart(id) }
const clearCart = () => { if (confirm('Xóa hết giỏ hàng?')) cartStore.clearCart() }
</script>